...
from (
    ...
    from sportscore_tennisevents a inner join tennis_api_wtatour t
    ...
    left join (
        ...
        from (
            ...
            from (
                ...
                ) a inner join tennis_api_wtamatch b on b.id::text=a.events_id
            ) c where period = 'all'
        ) aa where stat_name != 'other'
        ) stats
        ) end_stats group by event
    ) s_stats on event=a.id::text
    where sport_id='2' and section_id='144' and status !='canceled'
) s
