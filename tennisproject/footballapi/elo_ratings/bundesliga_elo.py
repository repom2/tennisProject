from footballapi.models import BundesligaElo, Bundesliga, Teams
...
def bundesliga_elorate():
    matches = Bundesliga.objects.filter(winner_code__isnull=False).order_by('start_at').distinct()

    matches = matches.filter(
            ~Exists(BundesligaElo.objects.filter(id=OuterRef('elo_rating_bundesliga'))))
    elo_table = BundesligaElo
