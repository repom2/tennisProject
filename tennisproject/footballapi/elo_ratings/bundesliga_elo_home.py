from footballapi.models import BundesligaEloHome, BundesligaEloAway,Bundesliga, Teams
...
def bundesliga_elo_home():
    matches = Bundesliga.objects.filter(winner_code__isnull=False).order_by('start_at').distinct()

    matches = matches.filter(
            ~Exists(BundesligaEloHome.objects.filter(id=OuterRef('elo_rating_home_bundesliga'))))
    ...
    for match in matches:
        try:
            ...
            continue
        home = BundesligaEloHome.objects.filter(team__id=match.home_team_id).order_by('-games')
        away = BundesligaEloAway.objects.filter(team__id=match.away_team_id).order_by('-games')
