from footballapi.models import LaLigaEloHome, LaLigaEloAway,LaLiga, Teams
...
def laliga_elo_home():
    matches = LaLiga.objects.filter(winner_code__isnull=False).order_by('start_at').distinct()

    matches = matches.filter(
            ~Exists(LaLigaEloHome.objects.filter(id=OuterRef('elo_rating_home_laliga'))))
    ...
    for match in matches:
        try:
            ...
            continue
        home = LaLigaEloHome.objects.filter(team__id=match.home_team_id).order_by('-games')
        away = LaLigaEloAway.objects.filter(team__id=match.away_team_id).order_by('-games')
