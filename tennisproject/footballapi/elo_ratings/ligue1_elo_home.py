from footballapi.models import Ligue1EloHome, Ligue1EloAway,Ligue1, Teams
...
def ligue1_elo_home():
    ...
    matches = matches.filter(
            ~Exists(Ligue1EloHome.objects.filter(id=OuterRef('elo_rating_home_ligue1'))))
    ...
    for match in matches:
        try:
            ...
            continue
        home = Ligue1EloHome.objects.filter(team__id=match.home_team_id).order_by('-games')
        away = Ligue1EloAway.objects.filter(team__id=match.away_team_id).order_by('-games')
