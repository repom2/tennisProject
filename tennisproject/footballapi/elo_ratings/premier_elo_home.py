from footballapi.models import PremierEloHome, PremierEloAway,PremierLeague, Teams
...
def premier_elo_home():
    matches = PremierLeague.objects.filter(winner_code__isnull=False).order_by('start_at').distinct()

    matches = matches.filter(
            ~Exists(PremierEloHome.objects.filter(id=OuterRef('elo_rating_home'))))
    ...
    for match in matches:
        try:
            ...
            continue
        home = PremierEloHome.objects.filter(team__id=match.home_team_id).order_by('-games')
        away = PremierEloAway.objects.filter(team__id=match.away_team_id).order_by('-games')
