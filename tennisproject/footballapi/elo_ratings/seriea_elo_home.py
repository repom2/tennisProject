from footballapi.models import SerieAEloHome, SerieAEloAway,SerieA, Teams
...
def seriea_elo_home():
    matches = SerieA.objects.filter(winner_code__isnull=False).order_by('start_at').distinct()

    matches = matches.filter(
            ~Exists(SerieAEloHome.objects.filter(id=OuterRef('elo_rating_home_seriea'))))
    ...
    for match in matches:
        try:
            ...
            continue
        home = SerieAEloHome.objects.filter(team__id=match.home_team_id).order_by('-games')
        away = SerieAEloAway.objects.filter(team__id=match.away_team_id).order_by('-games')
