from icehockeyapi.models import MestisEloHome, MestisElo, MestisEloAway, Mestis, Teams
...
def mestis_elo_home():
    matches = Mestis.objects.filter(winner_code__isnull=False).order_by('start_at').distinct()

    matches = matches.filter(
            ~Exists(MestisEloHome.objects.filter(id=OuterRef('elo_rating_mestis_home'))))
    ...
    for match in matches:
        try:
            ...
            continue
        home_home = MestisEloHome.objects.filter(team__id=match.home_team_id).order_by('-games')
        away = MestisElo.objects.filter(team__id=match.away_team_id).order_by('-games')

        home = MestisElo.objects.filter(team__id=match.home_team_id).order_by('-games')
        away_away = MestisEloAway.objects.filter(team__id=match.away_team_id).order_by('-games')
        ...
        if not home_home:
            ...
            home_elo = home_home[0].elo
            home_games = home_home[0].games
        if not away_away:
            away_elo = 1500
            away_games = 0
        else:
            away_elo = away_away[0].elo
            away_games = away_away[0].games
        ...
        # Loser
        #prob = probability_of_winning(home_elo, away_elo)
