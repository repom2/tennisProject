...
from tennisapi.ml.utils import define_query_parameters, probability_of_winning
...
def predict(level, tour):
    ...
    columns = ["winner_fullname", "loser_fullname"]
    logging.info(
    ...
    for i in range(0, 50):
        ...
        stats_params = params
        stats_params["start_at"] = "2025-01-01"
        ...
        data[["stats_win"] + stats_win_cols] = data.apply(
            lambda x: match_prob(
                x.player1 if x.player1 else 0,
                x.player2 if x.player2 else 0,
                sets=sets,
            ),
            axis=1,
        ).round(2)
        ...
        if use_scrape:
            if level == "atp":
                ...
                time.sleep(4)
                data[scrape_data_away] = data.apply(
            ...
            data[scrape_data_home] = None
            data[scrape_data_away] = None
        data["player1"] = data.apply(
            lambda x: (
                tour_spw + (x.home_spw - tour_spw) - (x.away_rpw - tour_rpw)
                if (x.away_rpw and x.home_spw)
        ...
        data[["stats_win_clay"] + stats_win_cols] = data.apply(
            lambda x: match_prob(
                x.player1_clay if x.player1_clay else 0,
                x.player2_clay if x.player2_clay else 0,
                sets=sets,
            ),
            axis=1,
        ).round(2)
        data[["stats_win_grass"] + stats_win_cols] = data.apply(
            lambda x: match_prob(
                x.player1_grass if x.player1_grass else 0,
                x.player2_grass if x.player2_grass else 0,
                sets=sets,
            ),
            axis=1,
        ).round(2)
