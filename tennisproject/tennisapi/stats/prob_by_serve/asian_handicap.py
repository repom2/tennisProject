...
def asian_handicap(set_score_probs):
    ...
    log.info("game differences for each set score: %s", game_diffs)
    ...
    log.info(
        f"Probability of the games total going over 21.5: {prob_over_215}"
    )
    log.info(
        f"Probability of the games total going over 22.5: {prob_over_225}"
    )
    log.info(
        f"Probability of the games total going over 23.5: {prob_over_235}"
    )

    return home_plus_75_handicap_prob, \
